...
\begin{proc}{individuoActualizaApuesta}{\In individuo: \nat, \In recursos: \TLista{\float}, \In cooperan: \TLista{\bool}, \Inout apuestas: \TLista{\TLista{\float}}, \In pagos: \TLista{\TLista{\float}}, \In eventos: \TLista{\TLista{\nat}}}{}
    ...
    \biggl(cooperan[individuo]=false \land (\forall j: \float) \bigl(0 \leq j < |apuestas| \implicaLuego  \\recursosNoCoopFinal(apuestas[j],pagos[individuo],eventos[individuo],recursos[individuo]) \leq \\recursosNoCoopFinal(apuesta,pagos[individuo],eventos[individuo],recursos[individuo])\bigr)
    \biggr)}
	
\aux{recursosNoCoopFinal}{\In apuesta: \TLista{\float}, \In pago: \TLista{\float}, \In evento: \TLista{\nat},\In recurso: \float}{\float}{ \\ \Bigl(recurso.\prod_{i=0}^{|evento|-1}\bigl(pago[evento[i]].apuesta[evento[i]]\bigr)\Bigr)}
...
{ \\
\sum_{i = 0}^{|cooperan|-1} \IfThenElse{cooperan[i]=true}{recursos[i]*apuestas[i][evento[i]]*pagos[i][evento[i]]}{0} *
\\ (\sum_{i = 0}^{|cooperan|-1} \IfThenElse{cooperan[i]=true}{1}{0})^{-1}}
\\ 
\aux{crecimientoCooperativo}{\In apuestas: \TLista{\TLista{\float}}, \In pagos: \TLista{\TLista{\float}}, \In eventos: \TLista{\TLista{\nat}},\In cooperan: \TLista{\bool},\In recursos: \TLista{\float}}{\float}{
\\
(\prod_{i=1}^{|eventos[0]|-1}\sum_{h = 0}^{|cooperan|-1} \IfThenElse{cooperan[h]=true}{apuestas[h][eventos[h][i]]*pagos[h][evento[h][i]]}{0}) *
\\ (\sum_{i = 0}^{|cooperan|-1} \IfThenElse{cooperan[i]=true}{1}{0})^{-(|eventos[0]|-1)}
} 
\\ 
