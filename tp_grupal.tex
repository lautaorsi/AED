...
\begin{proc}{trayectoriaExtrañaEscalera}{\In trayectoria: \TLista{\float}}{\bool}
	...
	\pred{CantidadMaximos}{\In lista: \TLista{\float}}{
	res = {\sum\limits_{i =1}^{|lista|-2}\Biggl(If\biggl(\Bigl(lista[i-1] < lista[i]\Bigr) \land \Bigl(lista[i] > lista[i+1]\Bigr)\biggr)Then(1)Else(0)\Biggr)}
} 
\end{proc}
